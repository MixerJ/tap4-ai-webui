...
import { BLOG_PAGE_SIZE, RevalidateOneHour, SITE_NAME } from '@/lib/constants';
...
export async function generateMetadata({
  ...
  const titleBase = t('title', { defaultValue: 'Blog' });
  ...
  const title = `${titleBase} - Page ${currentPage} - Toolsify AI`;

  return {
    title,
    description,
    keywords,
    alternates: buildAlternates(locale, `blog/page/${currentPage}`),
    ...buildSocialMetadata({
      locale,
...
export default async function BlogPagePage({
  ...
  const canonicalUrl = buildLocalizedUrl(locale, `blog/page/${currentPage}`);
  const blogJsonLd = {
